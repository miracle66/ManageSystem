...
(function( factory ){
   ...
   $.extend( altEditor.prototype, {
       ...
       _openEditModal: function ( )
       {
         ...
         for( var i = 0; i < dt.context[0].aoColumns.length; i++ )
         {
          ...
          $('#altEditor-modal').on('show.bs.modal', function() {
            $('#altEditor-modal').find('.modal-title').html('编辑');
            $('#altEditor-modal').find('.modal-body').html('<pre>' + data + '</pre>');
            $('#altEditor-modal').find('.modal-footer').html("<button type='button' data-content='remove' class='btn btn-default' data-dismiss='modal'>关闭</button>\
               <button type='button' data-content='remove' class='btn btn-primary' id='editRowBtn'>保存</button>");
          });
        ...
        * @Lily
        */
       _ajaxData: function(api_url, param, type, dataType, get_data){
           $.ajax({
               url: api_url,
               data: param,
               type: type,
       ...
       _getData: function (url,data,callback) {
           var me = this;
           var dt = this.s.dt;
           me._ajaxData(url,data,"POST","JSON",function get_data(data){
               var obj = data.status;
       ...
       _editRowData: function()
       {
         ...
         $('form[name="altEditor-form"] input').each(function( i )
         {
           ...
           if(isValidate == 1){
               ...
               that._getData(that.c.ajax,{data:data,action:'edit'},function(status){
                   if(status==1){
       ...
       _openDeleteModal: function ()
       {
         ...
         for( var i = 0; i < dt.context[0].aoColumns.length; i++ )
         {
          ...
          $('#altEditor-modal').on('show.bs.modal', function() {
            $('#altEditor-modal').find('.modal-title').html('删除');
            $('#altEditor-modal').find('.modal-body').html('<pre>' + data + '</pre>');
            $('#altEditor-modal').find('.modal-footer').html("<button type='button' data-content='remove' class='btn btn-default' data-dismiss='modal'>关闭</button>\
       ...
       _deleteRow: function( )
       {
         ...
         that._getData(that.c.ajax,{data:data,action:'delete'},function(status){
           if(status==1){
       ...
       _openAddModal: function ( )
       {
         ...
         for( var i = 0; i < dt.context[0].aoColumns.length; i++ )
         {
          ...
          $('#altEditor-modal').on('show.bs.modal', function() {
            $('#altEditor-modal').find('.modal-title').html('新增');
            $('#altEditor-modal').find('.modal-body').html('<pre>' + data + '</pre>');
            $('#altEditor-modal').find('.modal-footer').html("<button type='button' data-content='remove' class='btn btn-default' data-dismiss='modal'>关闭</button>\
        ...
        // console.log('add row')
         ...
         $('form[name="altEditor-form"] input').each(function( i )
         {
           ...
           if(isValidate == 1){
               ...
               that._getData(that.c.ajax,{data:data,action:'add'},function(status){
                   if(status==1){
